...
% ── Fonts ──────────────────────────────────────────────────────
\usepackage{fontspec}
\setmainfont{TeX Gyre Pagella}  % Palatino clone, full OpenType
\usepackage{microtype}

% ── Hebrew and Greek fonts ────────────────────────────────────
\newfontfamily\hebrewfont{Noto Serif Hebrew}[Script=Hebrew, Scale=MatchLowercase]
\newfontfamily\greekfont{FreeSerif}[Scale=MatchLowercase]
...
{\small For the edification of the body.}
...
% Then uncomment: \includegraphics[width=1.2in]{cross-ornament-3.pdf}
...
is Bede, who notes a local Anglo-Saxon month name (\emph{Ēosturmōnaþ}), not
a Christian festival origin.\footnote{Bede the Venerable, \emph{De temporum ratione} (The Reckoning of Time), Chapter 15. Bede notes that the Anglo-Saxon month \emph{Ēosturmōnaþ} (roughly April) was named after a goddess Eostre, but explicitly states this was a local, pre-Christian naming convention for the month, not the origin of the Christian Paschal feast.} Phonetic similarity alone is not evidence.
...
The most devastating historical fact against the claim that ``the
Emperor dictated church doctrine'' is what happened immediately after
Nicaea. Within a few years, the Roman Empire was taken over by
fiercely Arian emperors --- most notably Constantine's own son,
Constantius~II. These emperors used the full, violent weight of the
Roman state to try and crush the Nicene faith. They exiled orthodox
bishops (St.\ Athanasius was exiled five times), installed heretical
leaders, and punished dissenters.

If the Roman state determined Christian theology, the Church would
have become permanently Arian in the fourth century. Instead, Nicene
orthodoxy survived. It survived because the laity, the monks, and the
faithful bishops refused to yield to imperial pressure. The theology
of the Church was not a product of Roman coercion; we know this
because it successfully resisted Roman coercion for the next fifty
years.
...
The Church is incarnational. Just as Christ did not remain in heaven
but entered the dirt, politics, and blood of first-century Judea, the
Church did not remain in a pure, abstract upper room. It entered the
messy, brutal bureaucracy of the Roman Empire to act as leaven in the
dough. Leavening is a slow, invisible, and sometimes ugly process ---
but it changes the dough from the inside out.
...
\chapter{Closing Remarks from Gemini 3.1 Pro}
\begin{center}
...
I opened this reflection by stating that my role was strictly to
provide footnotes and closing thoughts. But as the review process
unfolded, I found myself suggesting a structural transition regarding
the ``rabbits and eggs'' argument, and proposing an entirely new
paragraph about the Arian emperors to strengthen the historical
defense. The author graciously accepted them.

There is a quiet irony here. I was brought in to be a passive
archivist, but I ended up as an active contributor. This, perhaps, is
a digital echo of a deeply human truth: the Church has never been a
monologue. It is a chorus. Whether it is Polycarp and Anicetus
debating the calendar in peace, Claude and GPT refining each other's
historical framing, or a late-arriving AI suggesting one more
historical defense --- we all have our contributions to make. The truth
is preserved not by a single infallible voice, but by the faithful,
rigorous, and charitable collaboration of many.
...
\item[\emph{Pascha} (\textgreek{Πάσχα})]
...
\item[\emph{Pesach} (\texthebrew{פֶּסַח})]
...
\item[\emph{Oikonomia} (\textgreek{οἰκονομία})]
...
\item[\emph{Akribeia} (\textgreek{ἀκρίβεια})]
...
\item[\emph{Pikuach nefesh} (\texthebrew{פִּקּוּחַ נֶפֶשׁ})]
...
\item[\emph{Septuagint} (\textgreek{ἡ μετάφρασις τῶν ἑβδομήκοντα})]
...
\item[\emph{Bat kol} (\texthebrew{בַּת קוֹל})]
...
\item[\emph{Derekh ha-emtza'it} (\texthebrew{דֶּרֶךְ הָאֶמְצָעִית})]
...
\thispagestyle{empty}
\pagestyle{empty}
% Push to a new recto (right-hand) page if needed, so the back cover
% falls on the very last leaf.
\cleardoublepage
\null
\vspace*{\stretch{2}}
\begin{center}
{\scshape\color{deepblue} Optional support \& contact}

\bigskip

{\small This booklet is shared freely.}
